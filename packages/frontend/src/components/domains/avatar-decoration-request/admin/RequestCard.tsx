...
export const RequestCard: React.FC<RequestCardProps> = ({ request: r, details, showPreview }) => {
  ...
  const approve = () => {
    const confirmed = confirm(`アバターデコレーションの申請「${r.name}」を承認し、Misskeyに追加します。本当によろしいですか？`);
    if (!confirmed) return;
  ...
  const reject = () => {
    const reason = prompt(`アバターデコレーションの申請「${r.name}」を却下します。本当によろしいですか？\n\n問題なければ、却下の理由を記入してください。\n（一部のMFMが利用できます。）`);
              ...
              <Stack gap={3}>
                ...
                {r.description && (
                  <div>
                    ...
                    <AutoCollapse>
                      <div className="text-muted border-start px-2 mb-0"><MfmView>{r.description}</MfmView></div>
                    </AutoCollapse>
                  </div>
                )}
              </Stack>
              ...
              <Table striped hover borderless className="mb-0 w-auto" style={{ '--bs-table-bg': 'transparent' } as any}>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{fileSizeInMB != null ? `${fileSizeInMB} MB` : <Spinner size="sm" />}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <th><i className="bi bi-chat-left-heart-fill" /> 説明</th>
                    <td>{r.description ? <MfmView>{r.description}</MfmView> : noText}</td>
                  </tr>
                  {r.status !== 'pending' && (
                    <>
                      <tr>
